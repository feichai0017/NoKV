...
func TestBuildPerasSealKeepsOnlyCommittedCertificates(t *testing.T) {
...
func TestBuildPerasSealOrdersByDependencyFrontier(t *testing.T) {
	firstPrepare := testSealPrepare()
...
func TestBuildPerasSealRejectsMissingOrMismatchedPrepare(t *testing.T) {
	prepare := testSealPrepare()
	commit := testCommitForPrepare(t, prepare)

	_, err := BuildPerasSeal(1, WitnessSnapshot{Commits: []CommitCertificateRecord{commit}})
	require.ErrorIs(t, err, ErrInvalidPerasSeal)

	commit.PrepareDigest[0] ^= 0xff
	_, err = BuildPerasSeal(1, WitnessSnapshot{
...
func TestBuildPerasSealRejectsMissingPredecessorAndCycles(t *testing.T) {
	firstPrepare := testSealPrepare()
...
func TestPerasSealMerkleStableAndSensitive(t *testing.T) {
	prepare := testSealPrepare()
	snapshot := WitnessSnapshot{
		Prepares: []PrepareRecord{prepare},
		Commits:  []CommitCertificateRecord{testCommitForPrepare(t, prepare)},
	}
	left, err := BuildPerasSeal(1, snapshot)
	require.NoError(t, err)
	right, err := BuildPerasSeal(1, snapshot)
	require.NoError(t, err)
	...
	changed, err := BuildPerasSeal(1, snapshot)
	require.NoError(t, err)
	require.NotEqual(t, left.CertificateMerkleRoot, changed.CertificateMerkleRoot)
}

func TestBuildPerasSealWithVersionsCarriesReplayRange(t *testing.T) {
	prepare := testSealPrepare()
	snapshot := WitnessSnapshot{
		Prepares: []PrepareRecord{prepare},
		Commits:  []CommitCertificateRecord{testCommitForPrepare(t, prepare)},
	...
	unversioned, err := BuildPerasSeal(1, snapshot)
	require.NoError(t, err)
	versioned, err := BuildPerasSealWithVersions(1, 100, snapshot)
	require.NoError(t, err)
...
func BenchmarkBuildPerasSeal64(b *testing.B) {
	snapshot := sealSnapshotForBench(b, 64)

	b.ReportAllocs()
	for b.Loop() {
		seal, err := BuildPerasSeal(1, snapshot)
		if err != nil {
			b.Fatal(err)
		}
