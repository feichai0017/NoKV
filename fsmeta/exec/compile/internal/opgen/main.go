...
func main() {
	...
	formatted, err = format.Source(source)
	if err != nil {
		fmt.Fprintf(os.Stderr, "peras-opgen: format read.peras.go: %v\n%s", err, source)
		os.Exit(1)
	}
	path = filepath.Join(*outDir, "read.peras.go")
	if err := os.WriteFile(path, formatted, 0o644); err != nil {
		fmt.Fprintf(os.Stderr, "peras-opgen: write %s: %v\n", path, err)
		os.Exit(1)
	}
...
func emitReadPrograms(readSpecs []specdsl.ReadSpec) ([]byte, error) {
	...
	b.WriteString("}\n\n")
	b.WriteString("func CompileDirectoryReadPlan(req fsmeta.ReadDirRequest, mount fsmeta.MountIdentity, includePeras, perasOnly bool) (DirectoryReadPlan, error) {\n")
	...
	b.WriteString("\t}\n")
	b.WriteString("\tif perasOnly {\n")
	b.WriteString("\t\tincludePeras = true\n")
	b.WriteString("\t}\n")
	b.WriteString("\tref := keyRef(KeyAccessReadPrefix, plan.ReadPrefixes[0])\n")
	b.WriteString("\tsource := DirectoryReadSourceBase\n")
	b.WriteString("\tif perasOnly {\n")
	b.WriteString("\t\tsource = DirectoryReadSourcePerasOnly\n")
	b.WriteString("\t} else if includePeras {\n")
	b.WriteString("\t\tsource = DirectoryReadSourcePerasOverlay\n")
	b.WriteString("\t}\n")
...
func emitCreate(spec specdsl.OpSpec) ([]byte, error) {
	...
	b.WriteString("\t\t// entry as a direct MVCC mutation. Local fsmeta opts into this path\n")
	b.WriteString("\t\t// via MaterializeSegments=true; distributed leaves it disabled.\n")
	b.WriteString("\t\tsegment.CanMaterialize = placement.CanSegment\n")
	b.WriteString("\t\tsegment.MaterializeInstall = SegmentInstallSingleBucket\n")
...
func emitAOTCompiledOpBody(b *bytes.Buffer) {
	...
	b.WriteString("\t\t// opt into MaterializeSegments=true to consume this path; distributed\n")
	b.WriteString("\t\t// runtimes leave MaterializeSegments=false and never enter materialize\n")
