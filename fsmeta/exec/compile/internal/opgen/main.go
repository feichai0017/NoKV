...
func main() {
	...
	for _, fileName := range files {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "peras-opgen: %v\n", err)
			os.Exit(1)
		}
		formatted, err := format.Source(source)
		if err != nil {
			fmt.Fprintf(os.Stderr, "peras-opgen: format %s: %v\n%s", fileName, err, source)
			os.Exit(1)
		}
		path := filepath.Join(*outDir, fileName)
		if err := os.WriteFile(path, formatted, 0o644); err != nil {
			fmt.Fprintf(os.Stderr, "peras-opgen: write %s: %v\n", path, err)
			os.Exit(1)
		}
	}
	...
	formatted, err := format.Source(source)
	if err != nil {
		fmt.Fprintf(os.Stderr, "peras-opgen: format dispatch.peras.go: %v\n%s", err, source)
		os.Exit(1)
	}
	path := filepath.Join(*outDir, "dispatch.peras.go")
	if err := os.WriteFile(path, formatted, 0o644); err != nil {
		fmt.Fprintf(os.Stderr, "peras-opgen: write %s: %v\n", path, err)
		os.Exit(1)
	}
...
func generatedFileName(spec specdsl.OpSpec) string {
	...
	return strings.ToLower(spec.Name) + ".peras.go"
}
...
func emitCreate(spec specdsl.OpSpec) ([]byte, error) {
	if len(spec.Effects) != 2 ||
		spec.Effects[0].ValueName != "DentryValue" ||
		spec.Effects[1].ValueName != "InodeValue" {
		return nil, fmt.Errorf("create spec must have dentry and inode effects")
	}
	var b bytes.Buffer
	writeGeneratedHeader(&b)
	b.WriteString("package compile\n\n")
	...
	fmt.Fprintf(&b, "\t\treturn %s{}, err\n", spec.ProgramType)
	b.WriteString("\t}\n")
	b.WriteString("\tplanKeys := [][]byte{dentryKey, inodeKey}\n")
	...
	b.WriteString("\t\tReadKeys:   planKeys[:1:1],\n")
	b.WriteString("\t\tReadPrefixes: [][]byte{},\n")
	b.WriteString("\t\tMutateKeys: planKeys[:2:2],\n")
	b.WriteString("\t}\n")
	...
	b.WriteString("\t\t\t{Kind: PredicateNotExists, Key: plan.MutateKeys[0]},\n")
	b.WriteString("\t\t\t{Kind: PredicateNotExists, Key: plan.MutateKeys[1]},\n")
	b.WriteString("\t\t},\n")
	b.WriteString("\t\tWriteEffects: []WriteEffect{\n")
	b.WriteString("\t\t\t{Kind: EffectPut, Key: plan.MutateKeys[0], Value: dentryValue},\n")
	b.WriteString("\t\t\t{Kind: EffectPut, Key: plan.MutateKeys[1], Value: inodeValue},\n")
	b.WriteString("\t\t},\n")
	...
	b.WriteString("\tif compiled.Delta.Kind != fsmeta.OperationCreate || len(compiled.Delta.ReadPredicates) != 2 || len(compiled.Delta.WriteEffects) != 2 {\n")
	b.WriteString("\t\treturn MaterializedOp{}, fsmeta.ErrInvalidRequest\n")
	b.WriteString("\t}\n")
	b.WriteString("\tdelta := compiled.Delta\n")
	b.WriteString("\tif values.DentryValue != nil || values.InodeValue != nil {\n")
	b.WriteString("\t\tif values.DentryValue == nil || values.InodeValue == nil {\n")
	b.WriteString("\t\t\treturn MaterializedOp{}, fsmeta.ErrInvalidRequest\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t\tdelta.WriteEffects = []WriteEffect{\n")
	b.WriteString("\t\t\t{Kind: EffectPut, Key: delta.WriteEffects[0].Key, Value: values.DentryValue},\n")
	b.WriteString("\t\t\t{Kind: EffectPut, Key: delta.WriteEffects[1].Key, Value: values.InodeValue},\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t}\n")
	...
	b.WriteString("\t\tReads:          refs[:2:2],\n")
	b.WriteString("\t\tWrites:         refs[2:4:4],\n")
	b.WriteString("\t\tConflictKeys:   refs[:4:4],\n")
	b.WriteString("\t\tEstimatedBytes: uint64(len(delta.ReadPredicates[0].Key) + len(delta.ReadPredicates[1].Key) + len(delta.WriteEffects[0].Key) + len(delta.WriteEffects[0].Value) + len(delta.WriteEffects[1].Key) + len(delta.WriteEffects[1].Value)),\n")
	b.WriteString("\t}\n")
	b.WriteString("\tfootprint.HasOpaqueKeys = refs[0].Opaque || refs[1].Opaque || refs[2].Opaque || refs[3].Opaque\n")
	b.WriteString("\tpredicates := []PredicateObligation{\n")
	b.WriteString("\t\t{Kind: PredicateNotExists, Key: delta.ReadPredicates[0].Key, NeedAbsent: true},\n")
	...
	b.WriteString("\t\t{ID: 0, Kind: EffectPut, Key: delta.WriteEffects[0].Key, Value: delta.WriteEffects[0].Value, Concrete: true, MountKeyID: dentryParts.MountKeyID, Bucket: dentryParts.Bucket, RecordKind: dentryParts.Kind, ValueHash: sha256.Sum256(delta.WriteEffects[0].Value)},\n")
	b.WriteString("\t\t{ID: 1, Kind: EffectPut, Key: delta.WriteEffects[1].Key, Value: delta.WriteEffects[1].Value, Concrete: true, MountKeyID: inodeParts.MountKeyID, Bucket: inodeParts.Bucket, RecordKind: inodeParts.Kind, ValueHash: sha256.Sum256(delta.WriteEffects[1].Value)},\n")
	b.WriteString("\t}\n")
	b.WriteString("\tatomicity := AtomicityGroup{Members: []MutationID{0, 1}, Recovery: RecoveryReplayAllOrNothing, Digest: digest}\n")
	...
	b.WriteString("\tif delta.WriteEffects[0].Kind != EffectPut || delta.WriteEffects[1].Kind != EffectPut || delta.WriteEffects[0].Value == nil || delta.WriteEffects[1].Value == nil {\n")
	b.WriteString("\t\treturn PlacementPlan{}, fsmeta.KeyParts{}, fsmeta.KeyParts{}, fsmeta.ErrInvalidRequest\n")
	b.WriteString("\t}\n")
	b.WriteString("\tdentryParts, ok := fsmeta.InspectKey(delta.WriteEffects[0].Key)\n")
	...
	b.WriteString("\t}\n")
	b.WriteString("\tinodeParts, ok := fsmeta.InspectKey(delta.WriteEffects[1].Key)\n")
	...
	b.WriteString("\tsegment := SegmentPlan{MergeKey: placement.MergeKey, Install: placement.Install, CanAppend: placement.CanSegment, RequiresMaterialize: placement.RequiresMaterialize, EstimatedPayloadBytes: footprint.EstimatedBytes, OperationCount: 1, MutationCount: 2}\n")
	b.WriteString("\tif placement.Install == SegmentInstallCatalog && placement.SingleBucket && len(placement.Buckets) == 1 {\n")
	...
	b.WriteString("\t\tsegment.MaterializeMergeKey = SegmentMergeKey{MountKeyID: placement.MountKeyID, PrimaryBucket: placement.Buckets[0], Install: SegmentInstallSingleBucket, Durability: placement.MergeKey.Durability, FormatVersion: placement.MergeKey.FormatVersion}\n")
	b.WriteString("\t}\n")
	...
	b.WriteString("\t}\n")
	b.WriteString("\treturn CompletionPlan{RetainCompletion: true, Kind: CompletionVisible, MutationCount: 2, DescriptorDigest: digest}\n")
	b.WriteString("}\n\n")
	...
	b.WriteString("\treturn []WatchProjection{{EventKind: WatchEventCreate, Key: delta.WriteEffects[0].Key, Parent: dentryParts.Parent, Name: dentryName(delta.WriteEffects[0].Key), Inode: inodeParts.Inode, EmitAt: WatchEmitVisible}}\n")
	b.WriteString("}\n")
	return b.Bytes(), nil
}
...
func writeGeneratedHeader(b *bytes.Buffer) {
	...
	b.WriteString("// Code generated by peras-opgen; DO NOT EDIT.\n\n")
}
