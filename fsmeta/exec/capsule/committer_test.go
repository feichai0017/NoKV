...
func TestDirectCommitterSubmitsSealsAndApplies(t *testing.T) {
	...
	seal, err := committer.CommitCapsule(context.Background(), opID("client-a", 1), deltaWithValueWrites("dentry/a", "inode=7"))
	require.NoError(t, err)
...
func BenchmarkDirectCommitterSingleOperation(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		if _, err := committer.CommitCapsule(ctx, OperationID{ClientID: "bench", Seq: uint64(i + 1)}, delta); err != nil {
			b.Fatal(err)
		}
	}
}
