...
func (s *pollingMirrorSource) ListCapsuleAuthorityGrants(context.Context, *coordpb.ListCapsuleAuthorityGrantsRequest) (*coordpb.ListCapsuleAuthorityGrantsResponse, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make([]*metapb.RootCapsuleAuthorityGrant, 0, len(s.grants))
	for _, grant := range s.grants {
		out = append(out, wire.RootCapsuleAuthorityGrantToProto(grant))
	}
