...
// to follow rooted Capsule authority grants.
...
// It owns one background stream and is intentionally limited to Capsule grants;
...
func (m *Mirror) run(ctx context.Context) {
	...
	for {
		if err := m.watch(ctx, &after); err != nil && ctx.Err() == nil {
			log.Printf("capsule authority mirror watch: %v", err)
		}
...
func (m *Mirror) poll(ctx context.Context) {
	for {
		if err := m.bootstrap(ctx); err != nil && ctx.Err() == nil {
			log.Printf("capsule authority mirror bootstrap: %v", err)
		}
		if !m.wait(ctx, m.interval) {
			return
		}
...
func (m *Mirror) bootstrap(ctx context.Context) error {
	resp, err := m.source.ListCapsuleAuthorityGrants(ctx, &coordpb.ListCapsuleAuthorityGrantsRequest{})
	if err != nil {
		return err
	}
...
func (m *Mirror) applyRootEvent(event rootevent.Event) {
	...
	case rootevent.KindCapsuleAuthorityGranted, rootevent.KindCapsuleAuthorityRetired:
		if err := m.table.ApplyRootEvent(event); err != nil {
			log.Printf("capsule authority mirror apply event kind=%d: %v", event.Kind, err)
		}
	}
}
