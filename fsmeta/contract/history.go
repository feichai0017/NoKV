...
func operationMayMutate(kind OperationKind) bool {
	switch kind {
	case OpCreate, OpUpdateInode, OpRename, OpRenameReplace, OpRenameSubtree, OpLink, OpUnlink, OpRemove, OpRemoveDirectory, OpOpenWriteSession, OpHeartbeatSession, OpCloseSession, OpExpireSessions:
		return true
	default:
		return false
	}
