...
func TestWitnessNodeDuplicatePrepareIsIdempotentButConflictingRecordFails(t *testing.T) {
	...
	digest, err := fsperas.SemanticDeltaPayloadDigest(conflicting.DeltaPayload)
	require.NoError(t, err)
...
func BenchmarkHolderSubmitWithWitnessNodes(b *testing.B) {
	...
	for b.Loop() {
		seq++
		id := fsperas.OperationID{ClientID: "bench", Seq: seq}
		commit, err := holder.Submit(context.Background(), id, delta)
		if err != nil {
			b.Fatal(err)
		}
		require.NoError(b, holder.MarkSealApplied(fsperas.PerasSeal{
			EpochID: 1,
...
func openBenchWitnessReplicas(b *testing.B, n int) ([]fsperas.WitnessReplica, func()) {
	b.Helper()
	now := time.Unix(100, 0)
...
func testPrepareRecord() fsperas.PrepareRecord {
	payload, err := fsperas.EncodeSemanticDeltaPayload(testSemanticDelta())
	if err != nil {
		panic(err)
	}
	...
	record := fsperas.PrepareRecord{
		EpochID:            1,
		OpID:               fsperas.OperationID{ClientID: "client-a", Seq: 1},
		DeltaPayload:       payload,
		DeltaDigest:        digest,
		DependencyFrontier: []fsperas.OperationID{},
...
func testCommitRecord(tb testing.TB, prepare fsperas.PrepareRecord) fsperas.CommitCertificateRecord {
	tb.Helper()
	digest, err := fsperas.PrepareDigest(prepare)
	require.NoError(tb, err)
	record := fsperas.CommitCertificateRecord{
