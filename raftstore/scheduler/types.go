...
// Mode classifies scheduler health. It is diagnostic only; stores must not
...
// Status captures local/control-plane scheduler health.
type Status struct {
	Mode              Mode      `json:"mode"`
...
// Client publishes store state to the control plane and returns any scheduling
// decisions that should be applied locally.
