...
func BuildReportWithConfig(cfg StatusConfig) (ReportResult, error) {
	...
	mode := describeMode(status, plan)
	result.Stage = mode.Stage
	result.Summary = mode.Summary
	result.NextSteps = mode.NextSteps
...
func describeMode(status StatusResult, plan PlanResult) modeReport {
	...
	case ModeStandalone:
		if plan.Eligible {
			return modeReport{
				...
				Summary: "workdir is still standalone and eligible to be promoted into a seed",
		...
		return modeReport{
			Stage:     "standalone-blocked",
			Summary:   "workdir is standalone, but migration preflight still has blockers",
			Next:      "nokv migrate plan",
	...
	case ModePreparing:
		return modeReport{
			...
			Next:    "retry nokv migrate init after inspecting partial migration state",
			NextSteps: []string{
				"nokv migrate status --workdir " + status.WorkDir,
	...
	case ModeCluster:
		return modeReport{
			...
			Next:    "nokv migrate expand | nokv migrate transfer-leader | nokv migrate remove-peer",
			NextSteps: []string{
	...
	default:
		return modeReport{
			Stage:   "unknown",
			Summary: fmt.Sprintf("unrecognized migration mode %q", status.Mode),
		}
	}
}
