...
use crate::metadata::chunk_apply_watch_keys;
...
fn open_persistent_holt_engine(
    ...
    let engine = nokv_raftnode::AppliedMetadataEngine::with_status(status, store.clone());
    (
        store.clone(),
        nokv_raftnode::PersistentAppliedMetadataEngine::new(
            engine,
