...
fn multi_region_bootstrap_requires_explicit_range() {
    ...
    assert!(err
        .to_string()
        .contains("requires NOKV_RAFTSTORE_REGION_RANGES"));
}
...
fn server_identity_rejects_zero_peer() {
    ...
    assert!(err.to_string().contains("NOKV_RAFTSTORE_PEER_ID"));
}
...
fn server_identity_rejects_invalid_bootstrap_flag() {
    ...
    assert!(err.to_string().contains("NOKV_RAFTSTORE_BOOTSTRAP"));
}
...
async fn wait_for_server(addr: SocketAddr) {
    ...
    panic!("raftstore test server at {addr} did not become ready");
}
