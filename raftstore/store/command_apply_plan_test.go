...
func TestCommandApplyDependenciesTrackPreparedMVCCInstall(t *testing.T) {
	req := &raftcmdpb.RaftCmdRequest{Requests: []*raftcmdpb.Request{testPreparedMVCCInstallRequest(
		[]byte("route"),
		[][]byte{[]byte("dep-a"), []byte("dep-b")},
		[]byte("entry-a"),
		[]byte("entry-b"),
	)}}
	...
	deps, barrier := commandApplyDependencies(req)
	require.False(t, barrier)
	require.Contains(t, deps, testUserWrite("route"))
	require.Contains(t, deps, testUserWrite("dep-a"))
	require.Contains(t, deps, testUserWrite("dep-b"))
	require.Contains(t, deps, testUserWrite("entry-a"))
	require.Contains(t, deps, testUserWrite("entry-b"))
}

func TestCommandApplyDependenciesTreatsInvalidPreparedMVCCInstallAsBarrier(t *testing.T) {
	req := &raftcmdpb.RaftCmdRequest{Requests: []*raftcmdpb.Request{{
...
func testPreparedMVCCInstallRequest(routingKey []byte, dependencyKeys [][]byte, entryKeys ...[]byte) *raftcmdpb.Request {
	entries := make([]*kvrpcpb.PreparedMVCCEntry, 0, len(entryKeys))
	for _, key := range entryKeys {
		entries = append(entries, &kvrpcpb.PreparedMVCCEntry{
...
func BenchmarkCommandApplyDependenciesPreparedMVCCInstall1000(b *testing.B) {
	keys := make([][]byte, 0, 1000)
	for i := range 1000 {
		keys = append(keys, []byte{byte(i >> 8), byte(i)})
	}
	req := &raftcmdpb.RaftCmdRequest{Requests: []*raftcmdpb.Request{testPreparedMVCCInstallRequest([]byte("route"), nil, keys...)}}
