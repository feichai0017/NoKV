...
use crate::codec::{decode_metadata_command_batch, encode_metadata_command_batch};
use crate::log::{MetadataRaftCommandBatch, MetadataRaftConfig, MetadataRaftEntry};
...
pub(crate) fn decode_metadata_raft_entry(
    payload: &[u8],
) -> Result<MetadataRaftEntry, MetadataRaftError> {
    match decode_record(payload)? {
        DecodedRaftRecord::Entry(entry) => Ok(entry),
        _ => Err(MetadataRaftError::Backend(
...
fn read_frame(file: &mut File) -> Result<FrameRead, MetadataRaftError> {
    ...
    if &header[..8] != FRAME_MAGIC {
        return Err(MetadataRaftError::Backend(
...
fn encode_entry_record(entry: &MetadataRaftEntry) -> Result<Vec<u8>, MetadataRaftError> {
...
fn encode_vote_record(vote: Option<Vote<u64>>) -> Result<Vec<u8>, MetadataRaftError> {
...
fn decode_record(payload: &[u8]) -> Result<DecodedRaftRecord, MetadataRaftError> {
            ...
            let payload = match input.u8()? {
                ...
                2 => {
                    ...
                    EntryPayload::Normal(MetadataRaftCommandBatch::new(commands).map_err(
                        |err| {
                            MetadataRaftError::Backend(format!("invalid raft command batch: {err}"))
                        },
                    )?)
                }
...
fn decode_membership(
    input: &mut Decoder<'_>,
) -> Result<Membership<u64, BasicNode>, MetadataRaftError> {
    ...
    for _ in 0..node_count {
        let node = input.u64()?;
        let addr = String::from_utf8(input.bytes()?).map_err(|err| {
            MetadataRaftError::Backend(format!("invalid raft node address: {err}"))
        })?;
...
fn decode_vote(input: &mut Decoder<'_>) -> Result<Option<Vote<u64>>, MetadataRaftError> {
    match input.u8()? {
        0 => Ok(None),
        1 => {
            let term = input.u64()?;
...
fn decode_optional_log_id(
    input: &mut Decoder<'_>,
) -> Result<Option<LogId<u64>>, MetadataRaftError> {
    match input.u8()? {
        0 => Ok(None),
...
fn decode_log_id(input: &mut Decoder<'_>) -> Result<LogId<u64>, MetadataRaftError> {
    let term = input.u64()?;
    let node_id = input.u64()?;
    let index = input.u64()?;
    Ok(LogId::new(
...
impl<'a> Decoder<'a> {
    ...
    fn u8(&mut self) -> Result<u8, MetadataRaftError> {
        let value = *self.input.get(self.offset).ok_or_else(|| {
            MetadataRaftError::Backend("file metadata raft log record is truncated".to_owned())
        })?;
        self.offset += 1;
        Ok(value)
    }
    ...
    fn len(&mut self) -> Result<usize, MetadataRaftError> {
        ...
        usize::try_from(raw).map_err(|_| {
            MetadataRaftError::Backend("metadata raft log length overflows usize".to_owned())
        })
    }
    ...
    fn take(&mut self, len: usize) -> Result<&'a [u8], MetadataRaftError> {
        let end = self.offset.checked_add(len).ok_or_else(|| {
            MetadataRaftError::Backend("file metadata raft log offset overflow".to_owned())
        })?;
...
fn push_len(out: &mut Vec<u8>, len: usize) -> Result<(), MetadataRaftError> {
    push_u64(
        ...
        u64::try_from(len).map_err(|_| {
            MetadataRaftError::Backend("metadata raft log length overflows u64".to_owned())
        })?,
    );
    Ok(())
}
...
fn log_storage_error(err: MetadataRaftError) -> StorageError<u64> {
    storage_error(ErrorSubject::Logs, ErrorVerb::Write, &err.to_string())
}

fn to_backend_error(err: impl std::error::Error) -> MetadataRaftError {
    MetadataRaftError::Backend(err.to_string())
}
