...
#[cfg(test)]
use nokvfs_meta::{NoKvFs, PreparedArtifact};
use nokvfs_object::{
...
#[cfg(test)]
pub(crate) struct LocalFuseBackend<M, O> {
    service: NoKvFs<M, O>,
}
...
#[cfg(test)]
impl<M, O> LocalFuseBackend<M, O> {
    pub(crate) fn new(service: NoKvFs<M, O>) -> Self {
        Self { service }
    }

    #[cfg(test)]
    pub(crate) fn service(&self) -> &NoKvFs<M, O> {
        &self.service
    }
}
...
#[cfg(test)]
impl<M, O> FuseBackend for LocalFuseBackend<M, O>
where
    M: MetadataStore + Send + Sync + 'static,
    O: ObjectStore + Send + Sync + 'static,
{
    type Prepared = PreparedArtifact;
    ...
    fn get_attr(&self, inode: InodeId) -> FuseBackendResult<Option<InodeAttr>> {
        self.service.get_attr(inode).map_err(Into::into)
    }
    ...
    fn read_symlink(&self, inode: InodeId) -> FuseBackendResult<Vec<u8>> {
        self.service.read_symlink(inode).map_err(Into::into)
    }
    ...
    fn update_root_attrs(&self, changes: UpdateAttr) -> FuseBackendResult<InodeAttr> {
        self.service.update_root_attrs(changes).map_err(Into::into)
    }
    ...
    fn update_attrs(
        ...
        self.service
            .update_attrs(parent, name, changes)
            .map_err(Into::into)
    }
    ...
    fn get_xattr(&self, inode: InodeId, name: &[u8]) -> FuseBackendResult<Option<Vec<u8>>> {
        self.service.get_xattr(inode, name).map_err(Into::into)
    }

    fn list_xattr(&self, inode: InodeId) -> FuseBackendResult<Vec<Vec<u8>>> {
        self.service.list_xattr(inode).map_err(Into::into)
    }

    fn remove_xattr(&self, inode: InodeId, name: &[u8]) -> FuseBackendResult<()> {
        self.service.remove_xattr(inode, name).map_err(Into::into)
    }
    ...
    fn prepare_artifact_replace(
        ...
        self.service
            .prepare_artifact_replace(parent, name)
            .map_err(Into::into)
    }
    ...
    fn read_session_object_blocks(
        ...
        self.service
            .read_session_object_blocks(output_len, blocks)
            .map_err(Into::into)
    }
