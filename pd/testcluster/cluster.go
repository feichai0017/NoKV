...
type Cluster struct {
	...
	RootStores map[uint64]*pdstorage.RootStore
	Services   map[uint64]*pdserver.Service
}
...
func OpenReplicatedWithTickIntervals(tb testing.TB, tickIntervals map[uint64]time.Duration) *Cluster {
	...
	c := &Cluster{
		...
		Services:   make(map[uint64]*pdserver.Service, 3),
	}
...
func (c *Cluster) LeaderService() (uint64, *pdserver.Service) {
	c.tb.Helper()
	id := c.WaitLeader()
