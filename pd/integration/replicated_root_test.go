...
func TestReplicatedRootAllocatorFencesPropagateToFollower(t *testing.T) {
	...
	allocResp, err := leader.AllocID(context.Background(), &pdpb.AllocIDRequest{Count: 4})
	require.NoError(t, err)
	require.Equal(t, uint64(4), allocResp.GetCount())
	lastID := allocResp.GetFirstId() + allocResp.GetCount() - 1

	tsoResp, err := leader.Tso(context.Background(), &pdpb.TsoRequest{Count: 3})
	require.NoError(t, err)
...
func TestReplicatedRootRemoveRegionPropagatesToFollowers(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		resp, err := cluster.Services[refreshingFollowerID].GetRegionByKey(context.Background(), &pdpb.GetRegionByKeyRequest{Key: []byte("m")})
		return err == nil && !resp.GetNotFound() && resp.GetRegionDescriptor().GetRegionId() == 211
	}, 8*time.Second, 50*time.Millisecond)

	_, err := leader.RemoveRegion(context.Background(), &pdpb.RemoveRegionRequest{RegionId: 211})
	require.NoError(t, err)
...
func TestReplicatedRootFollowerReadCanBeStaleUntilReload(t *testing.T) {
	...
	_, err := leader.RemoveRegion(context.Background(), &pdpb.RemoveRegionRequest{RegionId: 212})
	require.NoError(t, err)

	staleResp, err := follower.GetRegionByKey(context.Background(), &pdpb.GetRegionByKeyRequest{Key: []byte("m")})
	require.NoError(t, err)
