...
Options:
  ...
  --root-peer <id=addr>     Meta-root gRPC peer mapping; repeatable (exactly 3)
...
if [[ "${#ROOT_PEERS[@]}" -ne 3 ]]; then
  nokv_die "serve-coordinator.sh: exactly 3 --root-peer values required (remote meta-root cluster)"
fi
