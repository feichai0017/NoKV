...
@dataclass(frozen=True)
class BatchLookupPlusResult:
    found: bool
    entry: DentryAttrPair | None
...
class NoKVFsMetaClient:
    ...
    def batch_lookup_plus(
        ...
        *,
        ...
        req = fsmeta_pb2.BatchLookupPlusRequest(
            mount=mount,
            snapshot_version=snapshot_version,
        )
        req.lookups.extend(
            fsmeta_pb2.LookupKey(parent=lookup.parent, name=lookup.name)
            for lookup in lookups
        )
        resp = self._stub.BatchLookupPlus(req, timeout=timeout)
        return [_batch_lookup_plus_result_from_proto(result) for result in resp.results]
...
def _batch_lookup_plus_result_from_proto(
    value: fsmeta_pb2.BatchLookupPlusResult,
) -> BatchLookupPlusResult:
    if not value.found:
        return BatchLookupPlusResult(found=False, entry=None)
    return BatchLookupPlusResult(
        found=True,
        entry=_pair_from_proto(value.entry),
    )
