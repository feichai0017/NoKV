...
from langgraph.checkpoint.nokv import InodeType, LookupKey, NoKVFsMetaClient
...
class FakeStub:
    def __init__(self):
        ...
        self.BatchLookupPlus = UnaryCall(
            fsmeta_pb2.BatchLookupPlusResponse(
                results=[
                    fsmeta_pb2.BatchLookupPlusResult(
                        found=True,
                        entry=fsmeta_pb2.DentryAttrPair(
                            dentry=fsmeta_pb2.DentryRecord(
                                parent=1,
                    ...
                    fsmeta_pb2.BatchLookupPlusResult(found=False),
                ]
            )
        )
...
def test_lookup_and_read_dir_plus_map_responses():
    ...
    batch = client.batch_lookup_plus(
        mount="vol",
        lookups=[LookupKey(parent=1, name="entry"), LookupKey(parent=1, name="missing")],
    ...
    batch_req, _ = stub.BatchLookupPlus.calls[0]
    ...
    assert batch_req.lookups[0].name == "entry"
    assert batch_req.lookups[1].name == "missing"
    assert batch_req.snapshot_version == 101
    assert batch[0].found is True
    assert batch[0].entry is not None
    assert batch[0].entry.inode.opaque_attrs == b"file"
    assert batch[1].found is False
    assert batch[1].entry is None
