...
class InstrumentedFsMetaClient:
    def __init__(self, inner: NoKVFsMetaClient) -> None:
        self.inner = inner
    ...
    def _call(self, op_name: str, fn: Callable[..., Any], **kwargs: Any) -> Any:
        ...
        try:
            ...
            self.operations.setdefault(op_name, OperationStats()).add(
    ...
    def to_json(self) -> dict[str, dict[str, float | int]]:
        return {
            name: stats.to_json()
            for name, stats in sorted(self.operations.items(), key=lambda item: item[0])
        }
...
def _count_layout_tree(*, target: str, mount: str, path: tuple[str, ...]) -> dict[str, int]:
