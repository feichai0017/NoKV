...
- Keep an eye on `NoKV.Stats.ValueLogGC` (for example `gc_runs` and `head_updates`); if compactions are generating discard stats but the value log head doesn’t move, GC thresholds may be too conservative.
