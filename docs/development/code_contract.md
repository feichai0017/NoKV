...
| `crates/nokvfs-meta/` | Metadata schema, Holt-friendly layout, `MetadataCommand`, Holt-backed metastore, in-process metadata service, snapshot retention pins, and object-reference GC queue policy. | Own provider-specific object behavior, FUSE/kernel cache policy, Python bindings, CSI behavior, or remote protocol compatibility shims. |
