...
| `crates/nokvfs-server/` | Long-running metadata service process, startup config, background GC ownership, health/status/control HTTP, inode-level metadata RPC, and future remote service boundary. | Own metadata semantics, durable layout, object provider internals, FUSE/kernel cache policy, or hidden compatibility shims. |
