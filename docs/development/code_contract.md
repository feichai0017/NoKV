...
| `crates/nokvfs-protocol/` | Storage-neutral metadata RPC DTOs shared by server and remote clients. | Execute metadata commands, own path resolution, import Holt, object-store clients, FUSE, metadata layout, or server/client implementations. |
| `crates/nokvfs-meta/` | Metadata schema, Holt-friendly layout, `MetadataCommand`, Holt-backed metastore, in-process metadata service, snapshot retention pins, object-reference GC queue policy, history pruning, and service-level GC workers. | Own provider-specific object behavior, FUSE/kernel cache policy, Python bindings, CSI behavior, or remote protocol compatibility shims. |
...
| `crates/nokvfs-client/` | Path-oriented Rust SDK over in-process or remote metadata services. | Own metadata layout, bypass `nokvfs-meta`, expose object-store internals, implement FUSE/kernel cache semantics, depend on `nokvfs-fuse`, or define metadata wire formats. |
...
| `crates/nokvfs-server/` | Long-running metadata service process, startup config, background GC ownership, health/status/control HTTP, framed metadata RPC, and future remote service boundary. | Own metadata semantics, durable layout, object provider internals, FUSE/kernel cache policy, or hidden compatibility shims. |
