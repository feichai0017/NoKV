...
现有分布式 KV 系统的控制面通常将 durable metadata truth、routing/scheduling service 与 singleton duties 绑定在同一个高可用单元中。这样的设计工程上直接、工业上常见，也足够有效；但它会带来一个长期被弱化的问题：一旦控制器承担全局唯一职责，例如 ID 分配、TSO 或 scheduler ownership，它的恢复就不再只是“重建一个读视图”，而变成“如何在不依赖本地 durable state 的情况下，从正确的全局下界继续服务”。
...
更精确地说，前两个条件定义 correctness boundary，后两个条件定义 usability boundary。truth/service 不分离，则可重建性没有清晰含义；没有 atomic lease-fence transition，则 singleton duties 的接管不安全；没有 freshness contract，则分离后的 stale view 无法显式表达；没有 windowing，则 remote rooted authority 虽然仍然正确，但会被逐次 fence write 打进热路径而失去工程可用性。
...
系统不再需要“拿到 lease 之后，再跨另一个 authority 或另一组键单独去找 allocator 下界”。ownership transfer 与 allocator lower bound 出自同一条 rooted truth，这就是 TOCTOU 窗口被结构性消除的原因。本文因此不把 recovery 描述成字面意义上的 “O(1) step”，而把它描述成：**recovery 所需的 critical information 来自同一份 rooted authority，无需跨多个独立 authority 组合恢复。**
...
这些数字说明的不是“NoKV 比 etcd 快”或“更慢”，而是：**serialized remote fence write 才是主要代价，windowing 才是让 rebuildable coordinator 可用的工程条件。** NoKV 的单次 remote rooted write 更重，因为它承载的是 typed rooted semantics、rooted state apply 与 checkpoint/compaction 相关开销，而 etcd CAS baseline 只是在更新单个 key。
...
CockroachDB 通过 range descriptor 与 gossip/lease 机制处理元数据传播与 stale routing，但其核心路径仍然是把 metadata service 深度嵌入数据平面 authority 中。与本文相比，CockroachDB 更强调 in-band metadata dissemination，而 NoKV 讨论的是 truth/service split 后的 rooted recovery boundary。
