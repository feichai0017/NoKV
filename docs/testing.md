...
| Control-plane degraded / Coordinator unavailable | Covered | `coordinator/adapter/scheduler_client_test.go`, `raftstore/store/command_service_test.go::TestStoreProposeCommandSurvivesSchedulerUnavailable`, `raftstore/integration/coordinator_degraded_test.go::TestClusterSurvivesCoordinatorUnavailableAfterStartup` | Covers both local degraded scheduler semantics and live multi-node Coordinator outage after route cache warmup; new cold-route misses still fail with `RouteUnavailable` as expected. |
