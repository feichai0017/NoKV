...
- [`engine/vlog/manager.go`](../engine/vlog/manager.go) – owns the open file set, rotation, and segment lifecycle helpers.
- [`engine/vlog/io.go`](../engine/vlog/io.go) – append/read/iterate/verify/sample IO paths.
...
- **Value-log manager (`engine/vlog/`)** – owns segment lifecycle (open/rotate/remove),
  encodes/decodes entries, and exposes append/read/sample APIs without touching
  MVCC or LSM policy.
- **File IO (`engine/file/`)** – mmap-backed `LogFile` primitives (open/close/truncate,
...
* Files are named `%05d.vlog` and live under `workdir/vlog/bucket-XXX/` when `Options.ValueLogBucketCount > 1`. [`Manager.populate`](../engine/vlog/manager.go) discovers existing segments at open.
* `Manager` tracks the active file ID (`activeID`) and byte offset; [`Manager.Head`](../engine/vlog/manager.go) exposes these so the manifest can checkpoint them (`manifest.EditValueLogHead`).
...
* `vlog.VerifyDir` scans all segments with [`sanitizeValueLog`](../engine/vlog/io.go) to trim corrupted tails after crashes, mirroring RocksDB's `blob_file::Sanitize`. Badger performs a similar truncation pass at startup.
...
1. **Append + Rotate** – [`Manager.AppendEntry`](../engine/vlog/io.go) encodes and appends into the active file. The reservation path handles rotation when the active segment would exceed `MaxSize`; manual rotation is rare.
2. **Crash recovery** – [`Manager.Rewind`](../engine/vlog/manager.go) truncates the active file and removes newer files when a write batch fails mid-flight. `valueLog.write` uses this to guarantee idempotent WAL/value log ordering.
3. **Safe reads** – [`Manager.Read`](../engine/vlog/io.go) returns an mmap-backed slice plus an unlock callback. Active segments take a per-file `RWMutex`, while sealed segments use a pin/unpin path to avoid long-held locks; callers that need ownership should copy the bytes before releasing the lock.
4. **Verification** – [`VerifyDir`](../engine/vlog/io.go) validates entire directories (used by CLI and recovery) by parsing headers and CRCs.
...
1. [`valueLog.write`](../vlog.go) builds a write mask for each batch, then delegates to [`Manager.AppendEntries`](../engine/vlog/io.go). Entries staying in LSM (`shouldWriteValueToLSM`) receive zero-value pointers.
...
* `nokv vlog --workdir <dir>` loads a manager in read-only mode and prints current head plus file status (valid, gc candidate). It invokes [`vlog.VerifyDir`](../engine/vlog/io.go) before describing segments.
