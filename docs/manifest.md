...
| `raftstore` | Does not own manifest state. Store-local region catalogs and raft WAL replay checkpoints live in `raftstore/meta`; runtime routing state lives in PD storage. |
