...
message WatchSubtreeRequest {
  ...
  // resume_cursor 为 0 表示从当前 commit ts 之后开始；非零表示从该 cursor 之后续传。
  RootCursor resume_cursor = 5;
...
enum WatchChangeKind {
  WATCH_CHANGE_KIND_UNSPECIFIED = 0;
  WATCH_CHANGE_KIND_CREATE = 1;
  WATCH_CHANGE_KIND_DELETE = 2;
  WATCH_CHANGE_KIND_UPDATE = 3;
}

message WatchEvent {
  RootCursor cursor = 1;          // commit ts/index 用于 resume
  WatchChangeKind kind = 2;
  // key 是 fsmeta key（D{...} / I{...}）的原始 bytes；客户端解码自行拿到
  // mount + parent + name 等结构化字段。
  bytes key = 3;
  // 旧值（DELETE/UPDATE 携带）/新值（CREATE/UPDATE 携带）。空表示该 side 无值。
  bytes prev_value = 4;
  bytes value = 5;
}
...
- `cursor` 由服务端的 raft commit (term, index) 直接给出 —— 与 `meta/root` `TailSubscription` 用同一个 cursor 模型，便于将来 unify。
- v0 不内置 server-side filter beyond prefix；将来要加 `WatchEventFilter`（按 op 类型 / 按事件类型）作为附加 oneof 字段，proto 演进兼容。
...
flowchart TB
  ...
  hook["raftstore: post-apply observer (per region)"]
  apply["raftstore apply loop: KvCommit / KvPrewrite-Rollback / ..."]

  apply -->|every committed mutation| hook
  hook -->|emit (cursor, key, op, prev, value)| router
...
raftstore 当前的 apply loop（`raftstore/peer/peer.go` + `raftstore/kv/apply.go`）按 raft 提交顺序逐个应用 mutation。Stage 2.2 加一个**post-apply observer**：

```go
// raftstore/peer/observer.go (new)
type ApplyObserver interface {
    OnApply(ctx context.Context, evt ApplyEvent)
}

type ApplyEvent struct {
    RegionID  uint64
    Cursor    rootstate.Cursor    // (term, applied-index)
    Mutations []ApplyMutation
}

type ApplyMutation struct {
    Op       OpKind   // PUT / DELETE
    Key      []byte
    PrevValue []byte
    Value    []byte
}
...
每个 region 的 peer 在 apply commit 之后、释放 latch 之前调用所有注册的 observer。**observer 调用必须 non-blocking**（observer 内部 goroutine 化或丢弃，详见 §6 back-pressure）。
...
| PR | scope | 测试 |
|---|---|---|
| **Stage 2.2-A** | raftstore peer post-apply observer interface + register/deregister + non-blocking dispatch + dropped metric | unit + raftstore/integration 加一个 observer 测试，断言 apply 顺序透传到 observer |
