...
> 状态：当前设计已经落地，相关实现分散在 `DB`、`raftstore`、`migrate`、`coordinator` 与 `meta/root` 中。本文重点不是复述命令，而是解释为什么 NoKV 没有把“单机版”和“分布式版”做成两套系统。
...
- 恢复、校验、观测、测试路径出现双轨制。
- 后面任何关于 snapshot、reshard、scheduler/control-plane 的研究都要分别做两遍。
...
sequenceDiagram
    ...
    MIG->>RS: serve / expand
    RS->>Coordinator: 加入 cluster control-plane
...
3. 向 `coordinator` / `meta/root` 注册 rooted truth。
...
2. 通过 `coordinator` proposal gate 进入 `meta/root`。
...
- workdir mode gate 已经进入正式实现。
- `migrate init` / `serve` / `expand` 已经是协议入口，不是临时脚本。
- `localmeta` 与 engine metadata 已明确分开。
- region snapshot 已是单独层，并被 migration / install 路径复用。
- 当前桥接路径已经能接入 `coordinator` / `meta/root` 主线。

## 9. 当前没有实现的能力

- promotion 阶段不做自动 split / rebalance。
- snapshot install 还不是 zero-copy table transfer 或 SST ingest 优化路径。
- failed `init` 后还没有单独的 rollback / repair 命令。
- migration observability 仍然偏基础。
- scheduler/control-plane runtime 还没有自动编排 migration 生命周期。
...
- 它让后续关于 control-plane、snapshot、调度和恢复的研究，都能建立在同一套系统之上
