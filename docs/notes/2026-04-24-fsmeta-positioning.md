...
> 状态：Stage 1 native-service roadmap v3。`fsmeta/` 已落 schema、plan、executor、跨 region 2PC 硬化、`ReadDirPlus`、native gRPC service + typed client。v3 相对 v2 的主要修订：
>
> - 新增 §7.5 主流分布式 KV 的 primitive 能力对比矩阵（NoKV vs TiKV / FoundationDB / etcd / CockroachDB）；
> - 新增 §7.6 三类结构性优势分类（Shape / Structure / Correctness），每类对应到已 / 将实现的原语；
> - §10 Related Work 扩充为 "工业 / 学术历史 + 近两年研究热点分级 + Stage 2 primitive × 研究锚点映射"；
> - 确认 Stage 1 benchmark 锚点为 "同一 NoKV 集群下 native API vs generic KV schema"，对标 workload 为 Mooncake / MLPerf Storage v2.0 checkpoint storm + Tectonic §3.2 fan-in。
>
> v2 修订（保留）：Stage 1 从 JuiceFS TKV adapter 改为 NoKV-native metadata service；JuiceFS 降级为 Stage 3 兼容性验证，不再作为主线卖点。
