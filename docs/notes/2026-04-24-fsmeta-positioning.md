...
3. **做 `fsmeta/exec` 到 `fsmeta/server` / `fsmeta/client` 的 native API**
   ...
   - `OpenWriteSession`；
   - `fsmeta/server` gRPC API，直接暴露 native `Create/Lookup/ReadDir/Unlink/Rename`，不要经过 TKV；
   - `fsmeta/client` typed wrapper，隐藏 generated proto client，给 demo / benchmark 暴露稳定 fsmeta record。
