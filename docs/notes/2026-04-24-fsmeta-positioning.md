...
> 状态：Stage 0 positioning v1。`fsmeta/` 已落包边界（`fsmeta/doc.go`）。v1 相对 v0 的主要修订：
>
> - 引入工业界 + 学术界完整参考坐标系；
> - `raftstore/client` 跨 region 2PC 现状校正（不是从零做，是硬化）；
> - 新增 §4.5 借鉴与差异化，每一条设计决定挂上学术/工业锚点；
> - 新增 §10 Related Work 完整引用清单。

## 导读

- 🧭 主题：NoKV 作为云原生分布式文件系统的 metadata service，是否是一个成立的生态位。
- 🧱 核心对象：`fsmeta`、`raftstore`、`percolator`、`coordinator`、`meta/root`、Succession。
- 🔁 调用链：filesystem frontend 或 JuiceFS adapter -> `fsmeta` schema/API -> NoKV transaction client -> raftstore multi-Raft execution；mount/subtree authority -> `meta/root` rooted events。
- 📚 参考对象：
...
| **JuiceFS + TiKV** | TKV 模式下 inode / dentry / chunk / session 编码成普通 KV。参考：[JuiceFS Internals](https://juicefs.com/docs/community/internals/)。 | JuiceFS 不要求 metadata backend 理解目录；TiKV 只是通用 range KV + tx。 | Stage 1 做 TKV-compatible adapter，用现有语义证明 NoKV 能承接真实 workload。 |
...
| Namespace shell | `namespace.go` 是 DB lifecycle shell；`fsmeta/doc.go` 已声明用户 metadata plane 包边界。 | 方向正确：`fsmeta` 是应用数据 plane，不放进内部 `meta/`。 |
...
```mermaid
flowchart TD
    F["Filesystem frontend / JuiceFS adapter / native API"] --> S["fsmeta schema and operations"]
...
- JuiceFS adapter 是验证入口，不是最终产品形态。
- native filesystem frontend 是后置目标，不能在 Stage 1 抢主线。
...
| AI checkpoint-aware metadata | Mooncake / Quiver (FAST'20, FAST'25) | Stage 3 储备；需要真实 workload trace。 |
...
| Measured effect | 引用 NoKV 已有 KV / scan / raftstore benchmark；单 fsync 域是结构性事实。 | 引用 JuiceFS 官方 metadata benchmark 的实测对比。 | "比 TiKV / FDB 更快"——在 Stage 2 benchmark 前都不能说。 |
...
| Non-goal | Stage 1 不做完整 POSIX FS，不替代 CephFS，不做数据块存储，不承诺 FUSE 性能。 | Stage 2 不做 kernel module / native client 数据面。 | 不要同时讲"教学平台"和"工业级 DFS"两套定位。 |
...
当前 `fsmeta/doc.go` 和本文（v1）已就位，Stage 0 闭合。

### Stage 1：JuiceFS adapter + transaction hardening

**关键校正**：`TwoPhaseCommit` 和 `AllocID / Tso` 都不是从零做，是**硬化 + 包装**。Stage 1 实际工程量比 v0 估计小一个量级。

必须做：

1. **硬化跨 region TwoPhaseCommit**  
   当前 `raftstore/client/client_kv.go:319` `TwoPhaseCommit` 有跨 region 分组和 primary-first 提交流程，但需要补：
   - secondary prewrite 失败后的 rollback / cleanup；
   - commit primary 成功、secondary commit 失败后的 resolve path；
   - region split / epoch mismatch / leader change 下的端到端测试；
   - transaction status / lock resolver 后台清理入口；
   - fsmeta rename / link / unlink 级语义测试。

2. **定义 fsmeta key schema v0**  
   先支持 JuiceFS-like TKV schema，而不是一上来发明全部原生协议：
   - `I{inode}`：inode attr；
   - `D{parent}{name}`：dentry；
   - `C{inode}{chunk}`：chunk mapping；
   - `S{session}`：session / liveness；
   - `U{dir}`：usage / quota counter；
   - `M{mount}`：mount-level record。

3. **做 JuiceFS TKV adapter POC**  
   目标是跑通 JuiceFS metadata benchmark，不是改 JuiceFS 语义。需要能回答：
   - create / lookup / readdir / rename / unlink 走 NoKV 是否正确；
   - 和 TiKV backend 比，慢在哪里，快在哪里；
   - hot directory、large directory、cross-directory rename 的 profile 是什么。
...
5. **补 distributed DeleteRange 设计草案**  
...
1. **云原生文件系统 metadata backend**  
   面向 JuiceFS / SeaweedFS-like frontend，提供 Go-native、Kubernetes-friendly、multi-Raft replicated metadata substrate。
...
3. 做 JuiceFS TKV adapter spike，只接最小 KV interface，先跑 create / lookup / readdir / rename。
4. 包装 `AllocID` / `Tso` 为 fsmeta 稳定 client surface（错误语义、超时、witness 失败路径）。
5. 定义 `ReadDirPlus` 的 native RPC 草案，但不要先实现。
6. 写 distributed DeleteRange design note，明确是否需要 rooted job。

这条线的判断标准很简单：Stage 1 如果不能让 JuiceFS metadata benchmark 跑起来，这个方向就只能停留在架构叙事；如果能跑起来并拿到 profile，Stage 2 的 metadata-native primitive 才有可信基础。
