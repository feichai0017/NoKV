...
3. **做 `fsmeta/exec` 到 `fsmeta/server` / `fsmeta/client` 的 native API**
   `fsmeta/exec` 已实现 `Create` / `Lookup` / `ReadDir` / `ReadDirPlus` / `Unlink` / `Rename`。`fsmeta/server` 暴露 FSMetadata gRPC API，`fsmeta/client` 提供 typed wrapper。`Rename` 已通过真集群跨 region integration test 消费 hardened 2PC。
...
- **AssertionNotExist 跨 region 多 key 原子**（已在 `percolator/txn.go` 和 `fsmeta/exec` 端到端实现）—— TiKV 有单 region 的；我们在跨 region 2PC 下保证多 key 原子 assertion，Create 和 Rename 的"多条必须都不存在"语义直接服务端跑完。
...
4. 定义 `fsmeta/server` gRPC API 和 `fsmeta/client` typed wrapper：`Create` / `Lookup` / `ReadDir` / `ReadDirPlus` / `Unlink` / `Rename`，错误语义按 fsmeta native contract 暴露。
