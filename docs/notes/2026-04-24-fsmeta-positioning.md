...
> 状态：Stage 1 native-service roadmap v4。`fsmeta/` 已落 schema、plan、executor、跨 region 2PC 硬化、`ReadDirPlus`、native gRPC service + typed client、formal benchmark（同集群 native vs generic-KV，ReadDirPlus 42.5×）。v4 相对 v3 的主要修订：
>
...
**三种身份共用同一份架构**：schema 不分 fs/object，`meta/root` namespace authority 事件不分 fs/object，Stage 2 五条 primitive 对三类消费者**全部有效**。这是 hyperscaler "一个 substrate, 多个市场"的标准打法。
...
1. **硬化跨 region TwoPhaseCommit**
   ...
   后续只补 fsmeta RenameSubtree / unlink 级语义测试和 transaction status / lock resolver 后台清理入口。

2. **定义 fsmeta native schema v0**
   `fsmeta/keys.go` 已定义 magic/version/key-kind；`fsmeta/value.go` 已定义 Inode / Dentry / Session 二进制 value codec。Chunk、quota usage、mount metadata 留到 Stage 2，不混进 Stage 1。

3. **做 `fsmeta/exec` 到 `fsmeta/server` / `fsmeta/client` 的 native API**
   `fsmeta/exec` 已实现 `Create` / `Lookup` / `ReadDir` / `ReadDirPlus` / `Unlink` / `RenameSubtree`。`fsmeta/server` 暴露 FSMetadata gRPC API，`fsmeta/client` 提供 typed wrapper。`RenameSubtree` 已通过真集群跨 region integration test 消费 hardened 2PC。
...
Stage 1 只是进入牌桌。NoKV 的独特点要到 Stage 2 才明显。每个原语都映射到学术 / 工业锚点；并且**每条原语对 DFS 和对象存储双消费者同时有效**：
...
| `RenameSubtree(src, dst)` 🚧 | Weil'04 subtree 思想 + CFS 单 shard 原子 | rooted subtree authority + bounded KV txn；Succession 保证 handoff legality，规避 Ceph 不稳定 | `mv /old /new` 跨目录 atomic | **S3 没有原生支持**：原子地把 `bucket/v1/*` 改名 `bucket/v2/*`——补工业普遍痛点 |
...
| `WatchSubtree(root, era)` 🚧 | **工业 / 学术全空白**（etcd watch 是 key 粒度，TiKV CDC 是 region 粒度） | `meta/root` tail subscription 模型迁移到数据面 changefeed，按 subtree 前缀过滤。**真正的 novel primitive。** | inotify / FUSE invalidate | bucket prefix 变更 → 触发下游 ETL pipeline；**S3 event notifications 的强一致版** |
| `SnapshotSubtree(root)` 🚧 | Mooncake dataset management；Colossus snapshot | subtree era + immutable frontier，不要求复制所有 dentry | dataset snapshot | bucket-level point-in-time view（替代 S3 versioning + 应用层一致性 hack） |
| `QuotaFence(root)` 🚧 | Tectonic / HopsFS quota counter；Succession authority | quota fence 作为 rooted event；普通计数走 KV，关键边界走 authority gate | 用户目录配额 | bucket 容量上限 + subtree 配额 |

**Stage 2 交付范围**：6 条里实际交付 2-3 条（优先 `ReadDirPlus`(已) + `WatchSubtree` + `SnapshotSubtree`），其余进 Stage 3 roadmap。避免 over-promise。
...
| **目录 prefix 共置 range partition** | ✅ 现有 + Stage 2 感知目录边界的 split-hint | ✅ 通用 range 分区，不感知目录 | ✅ directory layer 有 path locality，不感知 hotspot | ❌ 单 boltdb 无分区 | ✅ 通用 range，不感知目录 |
...
| **Succession-bounded authority handoff** | ✅ TLA+ 形式化 + 4 条最小保证 + 4 个对照反例 | ❌ PD 选主无形式化 handoff | ❌ 控制器容错但无形式化 | ❌ raft 选主，无 succession 概念 | ❌ range lease transfer 无形式化 |
| **WatchSubtree / RenameSubtree / SnapshotSubtree / QuotaFence** | 🚧 Stage 2 规划 | ❌ 无对应 | ❌ 无对应 | ⚠️ key-prefix watch 最接近但非 subtree 语义 | ❌ 无对应 |

**这张表最强的三行**是 WatchSubtree / Succession / 单 fsync 域 —— NoKV 在这三条上**结构性独有**，不是"实现更优"。
...
- `BatchCreate`（Stage 2）—— checkpoint storm 的自然形状；单 primary 覆盖 N 文件。
...
- **目录前缀感知 region split**（Stage 2）—— range partition 不是新，**split 策略感知目录边界是新**；Tectonic FAST'21 §3.2 公开承认 hash-by-dir 解决不了 fan-in hotspot。
...
- **Succession**（已 TLA+ 证明）—— `spec/Succession.tla` 正模型 + 四个对照反例模型（LeaseOnly / TokenOnly / ChubbyFencedLease / LeaseStartOnly）。**别人都有 TLA+，但没人把 handoff legality 抬成独立的 correctness class。**
...
- **未来**：Succession 扩展到 namespace authority（mount / subtree / snapshot / quota handoff）—— DaisyNFS 风格的分布式 metadata verification 是学术空白。
...
4. **多租户 mount / subtree authority service**
   把 Succession 从 coordinator authority 推广到 namespace authority：mount owner、snapshot era、quota fence、subtree migration 都有明确 handoff contract。这一条被 1/2/3 都消费，不是独立产品。
...
1. 写 `fsmeta` v0 schema 和 operation contract：`Create` / `Lookup` / `ReadDir` / `RenameSubtree` / `Unlink` / `OpenWriteSession`。
...
3. 补 `fsmeta/exec` 的 `Unlink` 和 `RenameSubtree`，让 native executor 真实消费跨 region 2PC。
4. 定义 `fsmeta/server` gRPC API 和 `fsmeta/client` typed wrapper：`Create` / `Lookup` / `ReadDir` / `ReadDirPlus` / `Unlink` / `RenameSubtree`，错误语义按 fsmeta native contract 暴露。
5. 提供 `cmd/nokv-fsmeta` 部署入口：只依赖 coordinator endpoint，启动 FSMetadata gRPC server。
6. 做 `benchmark/fsmeta`：同一 NoKV 集群下 generic KV schema vs fsmeta native service，并提交可追溯结果 CSV。
7. 包装 `AllocID` 为 fsmeta 稳定 client surface（如果后续由 fsmeta service 自己分配 inode）。
8. 做 `cmd/fsmeta-demo`：模拟 checkpoint storm、dataset readdir、跨目录 atomic rename、quota check。
9. 写 distributed DeleteRange design note，明确是否需要 rooted job。
...
- **NoKV Succession TLA+ + DaisyNFS 风格扩展到分布式 metadata service 语义**是明确空白点，可投 OSDI / SOSP。

Deterministic simulation testing：
- **FoundationDB flow simulation** —— 业界 canon。
- **TigerBeetle / DBOS / LazyFS** —— 近两年复刻浪潮。
- NoKV 现有 **18 VFS failpoint + 8 raft failpoint + Succession 对照模型**已经是 deterministic simulation 的雏形；整理发表是一篇 ATC / NSDI workshop 级文章。

**Tier 3 — 借思想不要直接做的方向**

Subtree 语义研究：
- **Weil SC'04**（Ceph 原生 paper）+ **ACM TOS'25** Yang et al., "Agile and Judicious Metadata Load Balancing" —— 20 年未解的活问题。Succession-bounded subtree handoff 有充分理由作为差异化。<https://dl.acm.org/doi/10.1145/3721483>
...
### 10.4 Stage 2 Primitive × 研究锚点映射

每条 Stage 2 primitive 都必须能精确引用 1-2 篇近 3 年论文作为正向或反向参考——**没有论文背书的 primitive 不做**。
...
| `RenameSubtree` 🚧 | Weil SC'04 subtree 思想；CFS single-shard 原子 | Ceph `tracker/#24840` + ACM TOS'25：20 年未修的 balancer 不稳定 | Succession 形式化 bounded handoff |
...
| `SnapshotSubtree` 🚧 | Mooncake (FAST'25) dataset management；IndexFS bulk insertion | Colossus snapshot 深度绑定 Bigtable tablet | subtree era + immutable frontier，rooted event 表达 |
| `QuotaFence` 🚧 | HopsFS (FAST'17) counter；Tectonic quota | Ceph quota 不形式化 → 并发下 race | quota fence = rooted authority event，Succession 保证切换合法 |
| `AssertionNotExist` 跨 region ✅ | — | FDB `add_conflict_range` 只能单 txn；TiKV 单 region | Percolator prewrite 阶段服务端检查，跨 region 多 key 原子 |

**原则**：Stage 2 任何一条 primitive 写代码前，先在 `docs/notes/` 落一份 design note，明确写出"借鉴了哪两篇论文的什么思想" + "我们加了什么是这两篇没做的"。没有这份 design note 的 primitive **不进 Stage 2 roadmap**。
