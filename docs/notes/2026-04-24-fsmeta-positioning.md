...
- **Eunomia**（已通过 TLC 有界模型检查）—— `spec/Eunomia.tla` 正模型 + 四个对照反例模型（LeaseOnly / TokenOnly / ChubbyFencedLease / LeaseStartOnly）。**别人都有 TLA+，但没人把 handoff legality 抬成独立的 correctness class。**
