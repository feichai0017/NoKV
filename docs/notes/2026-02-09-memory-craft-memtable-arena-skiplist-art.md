...
* 前缀压缩减少重复路径。
* leaf 同时保存两份键：
  - `route key`：只给 trie 路由与顺序查找使用
  - `original internal key`：给 memtable / iterator / 上层系统返回
* ART 是 **internal-key-only** 的 memtable 索引，不再兼容 raw key 路径。
...
NoKV ART 的关键不是“树结构本身”，而是主 `COW + CAS` 模型下的克制优化：

* 读路径读不可变快照，不做 full OLC 的 read-side version validate。
* 大多数结构更新仍然是复制 payload / node 后 CAS 安装。
* 当前只保留一个很窄的 writer-side `OLC-lite` fast path：`replaceChild` 可原地替换单个 child pointer。
* 冲突时仍然重试 `tryInsert`。

这比 full OLC 更容易维护，也更符合 NoKV 当前的工程目标：先保证 internal-key 语义、可维护性，再拿稳定的热点收益。

### 6.3 查询与范围扫描

`lowerBound` 统一了 `Search` 与 `Seek` 的行为语义，也让范围扫描路径更自然。但要明确：

* `ART` 更擅长 `Insert / Get / Seek`
* 纯 `IteratorNext` 仍然通常弱于 skiplist
* 当前实现是用更高的 memindex 内存去换热点路径性能，不是“白赚”
...
| Insert / Get / Seek | 中 | 强 |
| 纯 IteratorNext | 强 | 中 |
| memindex 内存 | 低 | 高（当前约 `2x` skiplist） |
| 并发写冲突行为 | CAS 重试、简单 | 主 `COW+CAS` + 窄 `OLC-lite` |
| 调试成本 | 低 | 高 |

工程建议：

* 如果系统默认目标是更好的 `Insert / Get / Seek`，当前 NoKV 选择 `ART` 是合理的。
* 如果更关注纯顺序迭代、实现简单度或 memindex 内存，`skiplist` 仍然是稳健基线。
* 不要把 “使用 ART” 当作算法创新；真正有价值的是它已经和 NoKV 的 internal-key、arena 生命周期和 LSM 读写路径绑定在一起。
...
事实：ART 的收益依赖负载形态与实现细节；当前 NoKV 里它明显改善 `Insert / Get / Seek`，但纯 `IteratorNext` 与 memindex 内存并不占优。
...
* Arena 决定上限性能是否稳定。
* Skiplist 决定基线是否稳健可控。
* ART 让 NoKV 在 memtable 热点路径上拥有一条区别于主流 skiplist 路线的工程选择，但它的代价和边界也必须被明确记录。
