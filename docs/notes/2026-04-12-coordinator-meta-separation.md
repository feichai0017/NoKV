...
- `meta/root/backend/replicated` 覆盖 root leader change：
  - old root leader 写入 `Tenure` 和 allocator fences
...
1. Coordinator 向 `meta/root` leader 提交 `KindTenure` campaign。
...
- `meta/root/event` 增加 `TenureGranted` rooted event。
- `meta/root/state` checkpoint state 增加 `Tenure`。
- local / replicated root backend 增加 `CampaignTenure`。
...
- `coordinator/server.Service` 增加显式 `ConfigureTenure(...)`。
...
- `RunTenureLoop(ctx)` 已落地；启用 `--coordinator-id` 的 coordinator 会后台续租。
- rooted `ReleaseTenure()` 已落地；graceful shutdown 会显式 release 当前 holder lease。
