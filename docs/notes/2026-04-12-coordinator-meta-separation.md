...
> 状态：设计说明，尚未实现。当前正式支持的部署仍然是 `single coordinator + local meta` 和 `3 coordinator + replicated meta`。本文讨论的是未来是否把 `coordinator` 和 `meta/root` 拆成独立进程、为什么拆、什么时候不该拆，以及如果拆必须补齐哪些协议边界。
...
- `meta/root` 对外 gRPC API。
- 独立 Coordinator 集群的 lease / owner 机制。
- ID window 预分配。

已经作为第一步实现：

- co-located 模式下的 TSO window 预分配。
...
- rooted lease event/state。
- lease acquire / renew / release。
- `Tso` / `AllocID` / scheduler operation gate。

测试：

- 同时两个 Coordinator 抢 lease，只有一个成功。
- active holder 存在时，`Campaign` 返回失败和 current holder。
- lease holder 失联后，新 holder 接管。
- 旧 holder lease 过期后，即使 window 未耗尽也拒绝发号。
- 本地时钟偏移在 `clock_skew_buffer` 内不会造成双 holder 发号。
- scheduler operation 只有 holder 返回。
...
nokv meta-root \
  --addr 127.0.0.1:2380 \
  --node-id 1 \
  --peer 1=127.0.0.1:2380 \
  --peer 2=127.0.0.1:2381 \
  --peer 3=127.0.0.1:2382 \
  --workdir ./artifacts/meta-1

nokv coordinator \
  --addr 127.0.0.1:2379 \
  --root-mode remote \
  --root-addr 127.0.0.1:2380,127.0.0.1:2381,127.0.0.1:2382 \
  --coordinator-id c-1
```
...
- 还没有 remote root 和 lease correctness 测试。
...
1. Allocator window 已先做进当前 co-located 模式；下一步再补 remote root 边界。
2. 再把 remote `VirtualLog` API 做成可测试边界。
3. 然后补 Coordinator lease。
4. 最后再开放 separated deployment。
