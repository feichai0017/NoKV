...
> 状态：分阶段实现中。当前默认正式模式仍然是 `single coordinator + local meta` 和 `3 coordinator + replicated meta`；`remote meta/root`、Coordinator lease gate、allocator window、remote root harness 已有最小实现，但 separated deployment 还不是默认运维路径。本文说明为什么拆、什么时候不该拆，以及拆分前必须补齐哪些协议边界。
...
- `nokv meta-root --metrics-addr ...` 会发布 `nokv_meta_root` expvar key。
