...
| --- | --- | --- |
| `cache.indexes` | Byte-budgeted W-TinyLFU cache for decoded table indexes (`fid` → `*pb.TableIndex`). | [`utils/cache`](https://github.com/feichai0017/NoKV/blob/main/utils/cache) |
| `blockCache` | Ristretto-based block cache (L0/L1 only) with per-table direct slots. | [`engine/lsm/cache.go`](https://github.com/feichai0017/NoKV/blob/main/engine/lsm/cache.go) |
| `cacheMetrics` | Atomic hit/miss counters for L0/L1 blocks and indexes. | [`engine/lsm/cache.go#L30-L110`](https://github.com/feichai0017/NoKV/blob/main/engine/lsm/cache.go#L30-L110) |
...
More on SST layout lives in [`docs/guide/manifest.md`](manifest.md) and [`docs/guide/architecture.md`](architecture.md#2-embedded-engine).
