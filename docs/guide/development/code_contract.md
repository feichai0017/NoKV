...
| `raftstore/` | Rust distributed data-plane target: OpenRaft isolation, mount-scoped replicated execution, raft log, Holt state-machine storage, snapshots, apply notifications, and the MetadataPlane/RaftAdmin/RaftTransport boundary. Temporary StoreKV compatibility belongs only in internal tests while the old surface is deleted. | Import Go fsmeta semantic packages, redesign public protobuf without a wire-compatibility plan, expose StoreKV/Percolator as a new mainline API, or revive deleted Go raftstore/txn/local/storage/experimental paths. |
