...
| `SubtreeAuthority.tla` | Namespace subtree authority handoff consumed by subtree rename. | One active authority, successor frontier coverage, sealed-reply rejection, predecessor finality. | `fsmeta/contract/*`, `fsmeta/exec/*_test.go`. |
...
| `FSMetaNamespace.tla` | Small root-directory namespace model for create/link/unlink/rename/snapshot/session. | Dentries point to live inodes, link counts match dentries, sessions target live non-expired inodes, snapshots only reference known inodes. | `fsmeta/contract/*`. |
