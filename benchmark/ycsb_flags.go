...
var (
	...
	fValueThreshold = flag.Int("value_threshold", 1024, "value size threshold (bytes) before spilling to the value log")
	...
	ycsbValueSize         = flag.Int("ycsb_value_size", 1000, "value size (bytes) for YCSB records (official defaults map to ~1KB records)")
