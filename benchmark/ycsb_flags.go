...
var (
	...
	ycsbWorkloads        = flag.String("ycsb_workloads", "A,B,C", "comma-separated YCSB workloads (A-F)")
	ycsbEngines          = flag.String("ycsb_engines", "nokv,badger", "comma-separated engines to benchmark (nokv,badger,rocksdb)")
	ycsbRecords          = flag.Int("ycsb_records", 100000, "number of records to preload during YCSB load phase")
	ycsbOperations       = flag.Int("ycsb_ops", 500000, "number of transactional operations per workload")
	ycsbConcurrency      = flag.Int("ycsb_conc", 8, "worker goroutine count for YCSB transactional phase")
