...
func (e *nokvEngine) printStats() {
	...
	for _, lvl := range snap.LSM.Levels {
		if lvl.Level == 0 {
			...
			l0Spill = lvl.SpillTables
			break
		}
	}
	fmt.Printf("[NoKV Stats] entries=%d l0_tables=%d l0_bytes=%d l0_spill=%d flush_pending=%d compaction_backlog=%d compaction_max=%.2f write_q=%d write_entries=%d write_bytes=%d throttle=%v vlog_segments=%d vlog_pending=%d vlog_discard=%d\n",
		snap.Entries,
		l0Tables,
		l0Bytes,
		l0Spill,
