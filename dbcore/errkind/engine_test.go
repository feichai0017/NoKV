...
func TestFromEngineMapsEmbeddedEngineErrors(t *testing.T) {
	tests := []struct {
		name string
		err  error
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.Equal(t, tt.kind, FromEngine(tt.err))
			require.Equal(t, tt.kind, FromEngine(fmt.Errorf("wrapped: %w", tt.err)))
		})
	}
}

func TestFromEngineHandlesNilAndUnknown(t *testing.T) {
	require.Equal(t, nokverrors.KindUnknown, FromEngine(nil))
	require.Equal(t, nokverrors.KindUnknown, FromEngine(fmt.Errorf("local unknown")))
}
