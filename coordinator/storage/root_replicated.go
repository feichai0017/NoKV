...
// metadata root node hosted by Coordinator.
...
// OpenRootReplicatedStore opens one Coordinator storage backend backed by the
...
func (cfg ReplicatedRootConfig) Validate() error {
	if strings.TrimSpace(cfg.WorkDir) == "" {
		return fmt.Errorf("coordinator/storage: replicated root mode requires workdir")
	}
	if cfg.NodeID == 0 {
		return fmt.Errorf("coordinator/storage: replicated root node id must be > 0")
	}
	if strings.TrimSpace(cfg.TransportAddr) == "" {
		return fmt.Errorf("coordinator/storage: replicated root mode requires transport address")
	}
	if len(cfg.PeerAddrs) != ReplicatedRootReplicaCount {
		return fmt.Errorf("coordinator/storage: replicated root mode requires exactly %d peer addresses", ReplicatedRootReplicaCount)
	}
	for id, addr := range cfg.PeerAddrs {
		if id == 0 {
			return fmt.Errorf("coordinator/storage: replicated root peer ids must be > 0")
		}
		if strings.TrimSpace(addr) == "" {
			return fmt.Errorf("coordinator/storage: missing replicated root peer address for node %d", id)
		}
	}
	if strings.TrimSpace(cfg.PeerAddrs[cfg.NodeID]) == "" {
		return fmt.Errorf("coordinator/storage: missing replicated root peer address for local node %d", cfg.NodeID)
	}
	return nil
}
