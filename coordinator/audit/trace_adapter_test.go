...
func TestDecodeReplyTraceAdapters(t *testing.T) {
	...
	readIndexRaw, err := json.Marshal([]map[string]any{
		{"member_id": "n1", "read_state_generation": 9, "successor_generation": 10, "accepted": true},
		{"member_id": "n2", "duty": "custom", "read_state_generation": 11, "successor_generation": 11, "accepted": false},
	})
	require.NoError(t, err)
	...
	require.Equal(t, uint64(9), decoded[0].CertGeneration)
	require.Equal(t, uint64(10), decoded[0].ObservedSuccessorGeneration)
	require.Equal(t, "custom", decoded[1].Duty)

	readIndexEnvelope, err := json.Marshal(map[string]any{
		"records": []map[string]any{
			{"read_state_generation": 12, "successor_generation": 13, "accepted": true},
		},
	})
	require.NoError(t, err)
	...
	require.Equal(t, uint64(12), decoded[0].CertGeneration)
